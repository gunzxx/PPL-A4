        ...
        <div class="card-container">
            <form class="form-container" method="POST" action="/{{ auth()->user()->getRoleNames()[0] }}/partners/offers/create">
                ...
                <div class="form-group">
                    <input class="form-input" required name="name" placeholder="Nama penawaran" id="name" cols="30" rows="10">
                    @error('name')
                        <p class="error">{{ $message }}</p>
                    @enderror
                </div>
                <div class="form-group">
                    <textarea class="form-input" required name="description" placeholder="Deskripsikan penawaran anda" id="description" cols="30" rows="10"></textarea>
                    @error('description')
                        <p class="error">{{ $message }}</p>
                    @enderror
                </div>
                <div class="form-row">
                    <div class="form-col">
                        <input class="numeric form-input @error('stok') invalid @enderror" value="{{ old('stok') }}" required name="stok" class="numeric" type="text" placeholder="Range stok (kg/bulan)">
                        @error('stok')
                            <p class="error">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="form-col">
                        <input class="numeric form-input @error('price') invalid @enderror" value="{{ old('price') }}" required name="price" class="numeric" type="text" placeholder="Harga kedelai (kg)">
                        @error('price')
                            <p class="error">{{ $message }}</p>
                        @enderror
                    </div>
