        ...
        <div class="card-container">
            <form class="form-container" method="POST" action="/{{ auth()->user()->getRoleNames()[0] }}/partners/agreements/create">
                ...
                <div class="form-group">
                    <input value="{{ old('bean_type') }}" class="form-input" required name="bean_type" placeholder="Masukkan jenis kedelai" id="bean_type" cols="30" rows="10">
                    @error('bean_type')
                        <p class="error">{{ $message }}</p>
                    @enderror
                </div>
                <div class="form-group">
                    <input value="{{ old('stok') }}" class="form-input numeric" required name="stok" placeholder="Masukkan stok kedelai" id="stok" cols="30" rows="10">
                    @error('stok')
                        <p class="error">{{ $message }}</p>
                    @enderror
                </div>
                <div class="form-group">
                    <input value="{{ old('price') }}" class="form-input numeric" required name="price" placeholder="Masukkan harga kedelai" id="price" cols="30" rows="10">
                    @error('price')
                        <p class="error">{{ $message }}</p>
                    @enderror
                </div>
                    ...
                    @if ($offer_details->count()>0)
                        <select class="form-input @error('bean_id') invalid @enderror" required name="offer_detail_id" id="bean_id">
                            @foreach ($offer_details as $offer_detail)
                                <option value="{{ $offer_detail->id }}">{{ ucfirst($offer_detail->petani->fullname) }} - {{ ucfirst($offer_detail->offer->name) }}</option>
                            @endforeach
                        </select>
                    @else
